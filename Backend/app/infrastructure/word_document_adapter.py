...
class WordDocumentAdapter(IDocumentGenerator):
    ...
    def generate_questions_document(self, quiz_data: Quiz) -> BytesIO:
        document = Document()
        document.add_heading('Examen de Evaluación', level=1)
        document.add_paragraph()

        for i, item in enumerate(quiz_data.questions, start=1): 
    ...
    def generate_answers_document(self, quiz_data: Quiz) -> BytesIO:
        document = Document()
